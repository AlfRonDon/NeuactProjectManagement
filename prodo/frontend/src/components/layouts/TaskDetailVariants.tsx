  ...
  return (
    <div className="h-[700px] rounded-xl overflow-hidden border border-neutral-200 bg-white flex flex-col">
      <div className="px-5 py-3 border-b flex items-center gap-3 shrink-0">
        ...
        <h3 className="text-sm font-bold text-neutral-900">Task Detail</h3>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="border border-neutral-200 rounded-xl p-3 relative">
              ...
              {showAssign && (
                <div className="absolute top-full left-0 mt-1 w-full bg-white border border-neutral-200 rounded-xl shadow-lg z-10 p-2">
                  ...
                  {PEOPLE.map(p => (
                    <button key={p} onClick={() => setShowAssign(false)}
                      ...
                      <span className={p === TASK.assignee ? "font-bold text-neutral-900" : "text-neutral-600"}>{p}</span>
                      {p === TASK.assignee && <Check className="w-3 h-3 text-green-500 ml-auto" />}
                    </button>
                  ))}
                </div>
              )}
  ...
  return (
    <div className="h-[700px] rounded-xl overflow-hidden border border-neutral-200 bg-white flex flex-col">
      <div className="px-5 py-3 border-b flex items-center gap-3 shrink-0">
        ...
        <h3 className="text-sm font-bold text-neutral-900">Dependencies</h3>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6">
        ...
        <div className="flex gap-3">
          <div className="flex-1 bg-green-50 rounded-xl border border-green-200 p-3 text-center">
            <div className="text-lg font-black text-green-700">{upstream.length}</div>
            <div className="text-xs font-semibold text-green-600">Depends on</div>
          </div>
          <div className="flex-1 bg-amber-50 rounded-xl border border-amber-200 p-3 text-center">
            <div className="text-lg font-black text-amber-700">{downstream.length}</div>
            <div className="text-xs font-semibold text-amber-600">Blocks</div>
          </div>
          <div className="flex-1 bg-red-50 rounded-xl border border-red-200 p-3 text-center">
            <div className="text-lg font-black text-red-700">{upstream.filter(t => t.status !== "done").length}</div>
            <div className="text-xs font-semibold text-red-600">Unresolved</div>
          </div>
        </div>
        ...
        <div className="bg-neutral-50 rounded-2xl border border-neutral-200 p-5">
          ...
          <div className="space-y-2 mb-4">
            ...
            {upstream.map(t => {
              ...
              return (
                <div key={t.id} className="flex items-center gap-3 bg-white rounded-xl p-3 border border-neutral-200">
          ...
          {/* Current task */}
          <div className="flex items-center gap-3 bg-blue-50 rounded-xl p-3 border-2 border-blue-300 my-2">
            <div className="w-2.5 h-10 rounded-full bg-blue-500" />
            <div className="flex-1">
              <div className="text-sm font-bold text-neutral-900">{TASK.title}</div>
            ...
            {downstream.map(t => {
              ...
              return (
                <div key={t.id} className="flex items-center gap-3 bg-white rounded-xl p-3 border border-neutral-200">
                  ...
                  <OctagonAlert className="w-5 h-5 text-red-400 shrink-0" />
                </div>
              );
            })}
            ...
            {downstream.map(t => (
              <div key={t.id} className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <span className="text-amber-600"> — currently {STATUS[t.status]?.label.toLowerCase()}, due {t.due}</span>
                </div>
              </div>
            ))}
            {indirect.length > 0 && (
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        <div className="bg-neutral-50 rounded-2xl border border-neutral-200 overflow-hidden">
          <div className="px-4 py-2.5 border-b flex items-center gap-2 bg-white">
            <span className="text-sm font-bold text-neutral-900">Project Timeline</span>
            <div className="flex-1" />
            ...
            {ACTIVITY_LOG.map((a, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-2">
                <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                  a.type === "done" ? "bg-green-500" : a.type === "comment" ? "bg-blue-400" : a.type === "status" ? "bg-amber-400" : "bg-neutral-300"
                }`} />
