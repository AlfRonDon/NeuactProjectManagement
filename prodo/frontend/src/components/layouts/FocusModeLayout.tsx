  ...
  return (
    <div className="h-[700px] rounded-lg overflow-hidden border border-neutral-200 bg-white flex flex-col">
      ...
      <div className="px-6 py-4 flex items-center gap-3 border-b">
        ...
        <span className="text-sm font-serif font-bold text-neutral-950">Focus Mode</span>
        <span className="text-xs text-neutral-400">CC v5</span>
        <div className="flex-1" />
        <div className="flex gap-1">
          {(["pick", "work", "done"] as const).map((p) => (
            <button key={p} onClick={() => setPhase(p)}
              className={`text-xs font-medium px-3 py-1.5 rounded-full capitalize transition-all ${phase === p ? "bg-neutral-900 text-white" : "text-neutral-400 hover:bg-neutral-100"}`}>
              {p}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        {phase === "pick" && (
          <div className="w-full max-w-lg">
            <div className="text-center mb-8">
              ...
              <h2 className="text-xl font-bold text-neutral-950">What should you work on?</h2>
            ...
            <div className="space-y-3">
              {aiPicks.map((task, i) => (
                <button key={task.id} onClick={() => { setFocused(task); setPhase("work"); }}
                  className="w-full text-left bg-neutral-50 hover:bg-blue-50 border border-neutral-200 hover:border-blue-300 rounded-lg p-4 transition-all group">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-blue-600 mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {phase === "work" && focused && (
          <div className="w-full max-w-2xl">
            <div className="text-center mb-6">
              <div className="inline-block text-xs font-bold uppercase tracking-wider bg-warn-bg text-warn-fg px-3 py-1 rounded-full mb-3">In Progress</div>
              <h2 className="text-2xl font-bold text-neutral-950">{focused.title}</h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-neutral-50 rounded-lg border p-4">
                ...
                {focused.depends_on.length > 0 ? (
                  <div className="text-xs text-bad-fg">Blocked by {focused.depends_on.length} task</div>
                ) : (
                  <div className="text-xs text-ok-fg flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> No blockers</div>
                )}
              </div>
              <div className="bg-neutral-50 rounded-lg border p-4">
            ...
            <div className="flex justify-center gap-3 mt-6">
              <button onClick={() => setPhase("done")} className="bg-ok-solid text-white text-sm font-medium px-6 py-2.5 rounded-lg hover:bg-ok-solid/90 transition-colors">
                Mark Complete
              </button>
              <button className="text-sm text-neutral-500 px-4 py-2.5 rounded-lg border hover:bg-neutral-50 transition-colors">
                I'm Blocked
              </button>
            </div>
          </div>
        ...
        {phase === "done" && (
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-neutral-950">Nice work!</h2>
            <p className="text-xs text-neutral-400 mt-1">Task completed. Here's what changed:</p>
            <div className="mt-4 bg-ok-bg border border-ok-solid/20 rounded-lg p-4 text-left max-w-sm mx-auto">
              <div className="text-xs text-ok-fg space-y-1">
                <div>{"\u2713"} Sprint progress: 22% {"\u2192"} 33%</div>
                <div>{"\u2713"} Unblocked: "E2E Testing"</div>
                <div>{"\u2713"} Burndown updated</div>
              </div>
            </div>
            <button onClick={() => setPhase("pick")} className="mt-6 bg-neutral-900 text-white text-sm font-medium px-6 py-2.5 rounded-lg hover:bg-neutral-800 transition-colors">
              Pick Next Task
            </button>
          </div>
        )}
