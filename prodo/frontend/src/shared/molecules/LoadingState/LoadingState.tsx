...
interface LoadingStateProps extends Omit<StackProps, 'color'> {
  label?: string | null
...
export function LoadingState({
  ...
  // Size configurations
  const sizeConfig = {
    small: { circular: 20, text: 'caption' as const },
    medium: { circular: 32, text: 'body2' as const },
    large: { circular: 48, text: 'body1' as const },
  }[size]
    ...
    return (
      <Fade in>
        <Stack
          ...
          role="status"
          aria-live="polite"
          sx={[{ minWidth: 0, width: centered ? '100%' : width, alignItems: 'center', justifyContent: centered ? 'center' : 'flex-start' }, ...sxArray]}
          {...props}
        >
 ...
 */
export function ContentSkeleton({ type = 'card', count = 1 }: ContentSkeletonProps) {
