...
function AdminPageWrapper({ greeting, nearestDeadline, user }: { greeting: string; nearestDeadline: number; user: any }) {
  ...
  const openCreateTask = () => setShowTask(true);

  return (
    <div className="fixed inset-0 bg-neutral-50 overflow-hidden flex flex-col">
      {/* ── UNIFIED HEADER — CMD style ── */}
      <div className="bg-white border-b border-neutral-100 px-4 py-1.5 flex items-center gap-3 shrink-0">
        <img src="/logo.png" alt="Logo" className="w-8 h-8 rounded-md shrink-0 object-cover" />
        <span className="text-lg font-serif font-semibold text-neutral-950 tracking-tight shrink-0">Portfolio Overview</span>
        ...
        <div className="flex-1 min-w-0">
          <div className="text-md font-semibold text-neutral-950">{greeting}, {user?.username || "Rohith"}</div>
        ...
        <CreateDropdown onCreateTask={openCreateTask} onCreateProject={() => setShowProject(true)} />
        <div className="w-px h-6 bg-neutral-200 shrink-0" />
      ...
      <div className="flex-1 min-h-0">
        <AdminOverview
          externalShowTask={showTask}
          externalShowProject={showProject}
          onCloseTask={() => setShowTask(false)}
          onCloseProject={() => setShowProject(false)}
        />
      </div>
      <VoiceControlBar />
    </div>
